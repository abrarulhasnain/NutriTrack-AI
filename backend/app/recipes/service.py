...
def _build_items_and_totals(db: Session, items_data):
    """
    Nutrition Engine for recipes.
    For each item, resolves the food reference, scales its nutrition
    values by the requested quantity, and accumulates recipe totals.
    """
    recipe_items = []
    totals = {
    ...
    for item in items_data:
        ...
        # Scale nutrition values based on requested quantity vs food's serving size
...
def create_new_recipe(db: Session, recipe_data: RecipeCreate, user_id: UUID):
...
def get_user_recipes(db: Session, user_id: UUID):
    return get_all_recipes(db, user_id)


def get_recipe(db: Session, recipe_id: UUID, user_id: UUID):
...
def update_existing_recipe(db: Session, recipe_id: UUID, user_id: UUID, recipe_data: RecipeCreate):
    recipe = get_recipe_by_id(db, recipe_id, user_id)
    if not recipe:
        raise ValueError("Recipe not found")
...
def delete_existing_recipe(db: Session, recipe_id: UUID, user_id: UUID):
    recipe = get_recipe_by_id(db, recipe_id, user_id)
    if not recipe:
        raise ValueError("Recipe not found")
